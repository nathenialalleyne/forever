...
### MRH-012: Define the diagnostic-only artifact boundary [Design ready; owner approval required]
...
- **Design output:** [`docs/compatibility/mrh-012-diagnostic-artifact-boundary.md`](compatibility/mrh-012-diagnostic-artifact-boundary.md). This is proposed and awaiting explicit owner approval; it is not `Done`. The source-backed closure currently rejects the full root prototype JAR and names the common/server-only diagnostic entrypoint contract without choosing a build architecture.
...
- **Current status and gate:** The documentation/design portion is ready for manual owner review, but MRH-010's source-vs-spec evidence gap remains open. Approval must separately accept the proposed diagnostic-only boundary and open MRH-013. This ticket is not `Done`; approval does not authorise Java, a build, an artifact, pack inclusion, publication, distribution, or a rights decision. The existing report cannot satisfy MRH-010 by rewiring alone, and no architecture choice is made here.
...
### MRH-013: Implement and package the approved diagnostic-only artifact [Blocked]
