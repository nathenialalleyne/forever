...
- **Current status and gate:** The documentation/design portion is ready for manual owner review. Approval must separately accept the proposed diagnostic-only boundary and open MRH-013. This ticket is not `Done`; approval does not authorise Java, a build, an artifact, pack inclusion, publication, distribution, or a rights decision.
...
- **Objective:** After explicit owner approval of MRH-012, select the smallest implementation architecture that satisfies the approved common/server-only boundary, implement MRH-010's read-only Matcha baseline diagnostic, and produce a separately reviewable artifact candidate. This is the implementation and packaging ticket, not another planning exercise.
- **Dependencies:** MRH-012 boundary approval (not merely design completion), MRH-010 (complete), PACK-01's tested baseline, ADRs 0020, 0024, 0025, 0026, 0027, and 0033, `docs/compatibility/mrh-012-diagnostic-artifact-boundary.md`, and the open rights/distribution decision where packaging or inclusion is proposed.
...
- **Acceptance criteria:**
  ...
  3. The candidate reports healthy, missing, decoy/MISSING, partially altered real-archive DEGRADED/WARN, unsupported, and malformed cases; all failure cases leave startup unblocked and world data unchanged.
...
- **Test requirements:** Architecture/closure and duplicate-entrypoint checks; existing unit tests; dedicated-server startup with a healthy archive, missing archive, decoy, partial real archive, unsupported and malformed evidence; reload lifecycle; no client-class loading; no registry/attachment/SavedData/network registration; before/after disposable-world comparison; exact artifact identity and provenance checks; removal and rights review. Do not use a real world.
...
- **Dependencies:** MRH-012 (diagnostic-only artifact boundary approved), MRH-013 (approved boundary implemented and artifact validated), MRH-010 (complete), PACK-01, ADR 0027, and `docs/decisions/OPEN-rights-model.md`.
...
- **Custom-code/artifact gate:** No new gameplay code is authorised. MRH-011 may package only the diagnostic-only artifact implemented and validated by MRH-013 after MRH-012 owner approval; the full root prototype JAR is explicitly not approved because it registers seven prototype systems at common initialization. The existing unit and GameTest evidence is preserved as behaviour evidence, not artifact-scope approval.
...
- **Blocked on:** MRH-012's boundary and owner approval, MRH-013's implementation and artifact validation, PACK-01's outstanding human real-play smoke test, and the open rights/distribution gates in `docs/decisions/OPEN-rights-model.md` and ADR 0020. Private-only preparation or a local artifact test does not authorise pack inclusion, publication, or distribution.
