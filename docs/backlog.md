...
### MRH-010: Fail loudly when the Matcha baseline is absent or altered [Verified in MRH-013 candidate; pack delivery pending]

- **Candidate resolution (2026-09-06):** MRH-013 satisfies the carried diagnostic contract in its separate dedicated-server artifact. [Canonical runtime evidence](compatibility/mrh-013-runtime-evidence.md) records locked-hash errors, distinct checksum mismatch, both one-sided archive cases, safe startup, lifecycle, healthy INFO-only output, and read-only/removal controls. The old prototype path remains unchanged. This is not pack delivery or client/integrated-server diagnostic coverage; MRH-011 remains blocked.
...
- **Historical prototype source-vs-spec gap (reviewed before MRH-013, 2026-09-06):** The following records why the prior completion claim was reopened. MRH-013 subsequently verified the separate candidate above; it did not repair this preserved prototype path. `MatchaServerEvidence.gather()` currently passes `null` for the observed digest and metadata and an empty fingerprint map, so its live result cannot prove the locked archive. `MatchaBaselineReport.fromStatus()` names the expected file, version, and roles in its missing remedies but omits the locked SHA-256. The current server-side healthy path remains unverified: namespaces and markers can at most establish data-side signals and cannot verify a remote client's resource role. Existing lifecycle tests also do not prove repeated-initialization idempotence. MRH-010 is not weakened: the missing message must name the expected locked SHA-256, a checksum mismatch must be distinct from Matcha parse errors, a one-sided role must be reported where detectable on that side, startup must remain unblocked without world writes, and a healthy install must produce no new noise. MRH-013 carries these exact acceptance and test requirements and must resolve them before MRH-011 shipping. Merely rewiring the existing report is insufficient. This is an evidence correction, not a superseding ADR or architecture decision.
...
### MRH-012: Define the diagnostic-only artifact boundary [Approved; implementation in MRH-013]
...
- **Design output:** [`docs/compatibility/mrh-012-diagnostic-artifact-boundary.md`](compatibility/mrh-012-diagnostic-artifact-boundary.md). The project owner approved this boundary on 2026-09-06 for private MRH-013 implementation and disposable-instance tests only. The source-backed closure rejects the full root prototype JAR and names the common/server-only diagnostic entrypoint contract. MRH-013 records the minimal build architecture separately before code.
- **Dependencies:** MRH-010 (requirements retained; source-vs-spec evidence gap open), ADRs 0020, 0024, 0025, 0026, 0033, and 0034, `docs/pivot/existing-code-inventory.md`, `docs/decisions/OPEN-rights-model.md`, and a review of `src/main/java/dev/forever/ForeverMod.java` and `dev.forever.compat.matcha`.
...
- **Custom-code gate:** The design-only gate is complete by explicit owner approval recorded in the design output. This ticket still authorises no Java, build, pack, artifact, or gameplay change; MRH-013 is the separate reviewed implementation ticket and must satisfy ADRs 0026 and 0033.
...
- **Current status and gate:** The project owner approved the diagnostic-only boundary and opened MRH-013 on 2026-09-06. MRH-010's source-vs-spec evidence gap remains open and must be resolved by MRH-013. Approval does not authorise gameplay, pack inclusion, publication, distribution, or a rights decision. The existing report cannot satisfy MRH-010 by rewiring alone; the selected architecture is recorded in [`mrh-013-diagnostic-architecture.md`](compatibility/mrh-013-diagnostic-architecture.md) before implementation.
...
### MRH-013: Implement and package the approved diagnostic-only artifact [Done: private dedicated-server candidate]

- **Acceptance evidence (2026-09-06):** [Canonical runtime and provenance record](compatibility/mrh-013-runtime-evidence.md). Source `729680b`, candidate SHA-256 `27fc15762d02583973ce7040ebd8dc6699cc528198c321f9cf7a7e7cb10569a4`. Independent final review found no technical blockers after nine exact-artifact server cases, client ResourceManager proof, and controlled read-only/removal checks. Source/fixture isolation and clean-CI tests are verified. Unsupported filesystem providers fail safely; client/integrated-server diagnostic execution and Windows/macOS support are not claimed. This completion authorises no pack inclusion, licence change, publication, or distribution.

- **Objective:** After explicit owner approval of MRH-012, select the smallest implementation architecture that satisfies the approved common/server-only boundary, implement MRH-010's read-only Matcha baseline diagnostic, and produce a separately reviewable artifact candidate. This is the implementation and packaging ticket, not another planning exercise. Internal implementation and artifact validation may proceed before PACK-01's human smoke; that smoke is a gate before MRH-011 includes the artifact.
- **Dependencies:** MRH-012 boundary approval (not merely design completion), MRH-010 (acceptance contract retained; evidence gap must be resolved here), PACK-01's recorded pinned-input/profile-assembly evidence (non-diagnostic prerequisites only, not PACK-01 completion or its final diagnostic criterion), ADRs 0020, 0024, 0025, 0026, 0027, and 0033, `docs/compatibility/mrh-012-diagnostic-artifact-boundary.md`, and the open rights/distribution decision where packaging or inclusion is proposed.
- **Authoritative sources to inspect:** The approved MRH-012 boundary, the existing `dev.forever.compat.matcha` source and tests, `src/main/resources/fabric.mod.json`, `src/main/java/dev/forever/ForeverMod.java`, `docs/compatibility/matcha-loading.md`, `matcha.lock.json`, `pack/`, `docs/pivot/existing-code-inventory.md`, `docs/world-save-safety.md`, and the provenance/licence records.
- **Existing-mod candidates:** None for the diagnostic boundary. Keep Global Packs as the existing loader; do not build a replacement loader or treat the full prototype JAR as a diagnostic artifact.
- **Custom-code/artifact gate:** **Owner approval recorded 2026-09-06.** The selected architecture and exact runtime closure are recorded in [`mrh-013-diagnostic-architecture.md`](compatibility/mrh-013-diagnostic-architecture.md) before implementation. Implement only the approved diagnostic. It must still satisfy ADRs 0026 and 0033, must not modify a third-party JAR, and must not silently activate or repurpose prototype gameplay systems. No implementation may treat the existing report wiring as sufficient for MRH-010's open evidence contract.
- **Explicit non-goals:** No gameplay feature, root `ForeverMod` artifact, prototype-system registration, client screen or entrypoint, persistent gameplay state, save migration, network protocol, Matcha extraction/repackaging, loader replacement, unrelated refactor, licence change, publication, or automatic distribution.
- **Acceptance criteria:**
  1. The owner-approved MRH-012 boundary and the selected minimal implementation architecture are recorded together before implementation; no unapproved architecture is inferred from the design document.
  2. The candidate artifact has one diagnostic-only common/server entrypoint, and a source/JAR closure check proves that `ForeverMod`, all seven prototype initializers, client registration, gameplay registries, persistent identifiers, and unrelated Matcha mapping material are absent or unreachable by the approved boundary. A separate pack/client verification covers the remote resource-pack role; healthy server evidence is not full dual-role proof, and no client feature is auto-approved.
  3. The candidate reports healthy, missing, decoy/MISSING, partially altered real-archive DEGRADED/WARN, unsupported, and malformed cases; all failure cases leave startup unblocked and world data unchanged. Before shipping, it must satisfy every MRH-010 acceptance exactly:
     1. A missing archive produces an unmistakable startup error naming the expected file and its locked SHA-256.
     2. An archive whose checksum differs from `matcha.lock.json` is reported distinctly from Matcha's own parse errors.
     3. A one-sided load, data without resource or the reverse, is reported where detectable on that side.
     4. The check never prevents the server from starting, and never modifies world data.
     5. The message names the remedy, not just the symptom.
     The healthy case must also demonstrate that a correct install produces no new diagnostic noise.
  4. The candidate's exact version, hash, source, licence/provenance, notices, removal notes, and relationship to the pinned Matcha archive are reviewed before any pack inclusion or distribution claim.
  5. Existing source, unit tests, GameTests, prototype tag, audit tool, generated evidence, and third-party provenance remain preserved and separately classified.
- **Test requirements:** Architecture/closure and duplicate-entrypoint checks; existing unit tests; unit tests for present, absent, altered, and unreadable archives; dedicated-server startup with a healthy archive, missing archive, decoy, partial real archive, unsupported and malformed evidence; a dedicated-server GameTest asserting the diagnostic appears; distinct checksum-mismatch versus Matcha parse-error reporting; one-sided role reporting where detectable; confirmation that a correct install produces no new noise; separate assembled-pack/client verification of the resource-pack role; no client feature approval from server evidence; reload lifecycle; no client-class loading; no registry/attachment/SavedData/network registration; before/after disposable-world comparison; exact artifact identity and provenance checks; removal and rights review; and an explicit repeated-initialization fixture proving exactly one registration set, one observation/report per `SERVER_STARTED` or successful reload, no report on failed reload, and no stale cross-server result. The canonical evidence link above distinguishes unit/callback fixtures, candidate-only server runs, and separate client/read-only/removal checks. Do not use a real world.
- **Removal/migration risks:** A candidate that accidentally retains a prototype registry or persistent identifier creates a save and removal obligation. A candidate that omits lifecycle reporting restores the silent LAB-02 path. Retain the approved boundary, source provenance, and recoverable build evidence; stop before pack inclusion if rights or artifact identity are unresolved.

### MRH-011: Ship the Matcha baseline diagnostic with the pack [Blocked]

- **Current checkpoint (2026-09-06):** MRH-012 owner approval and MRH-013 private candidate validation are complete; see [canonical evidence](compatibility/mrh-013-runtime-evidence.md). The evidence below describes the historical prototype experiment. Remaining gates are PACK-01's human real-play smoke, authorised artifact transport/provenance and rights/distribution decisions, and this ticket's own assembled-pack proof. No diagnostic has been added to `pack/`. The candidate is dedicated-server-only and requires a supported filesystem provider; broader deployment must not be assumed.
...
- **Dependencies:** MRH-012 (diagnostic-only artifact boundary approved), MRH-013 (approved boundary implemented and artifact validated, including the MRH-010 acceptance contract), MRH-010 (all acceptance and test requirements complete), PACK-01's recorded non-diagnostic pinned-input/profile-assembly evidence and human smoke gate before inclusion (not PACK-01 completion), ADR 0027, and `docs/decisions/OPEN-rights-model.md`.
- **Evidence for this ticket:** Two failure paths were tested against a server assembled from `pack/`, and both are silent.
  ...
  **Artifact-scope qualification:** The end-to-end experiment below used the existing companion build as behaviour evidence only. It is not approval to ship that full root JAR. `src/main/java/dev/forever/ForeverMod.java:41-63` unconditionally initializes equipment, mastery, settlement, storage, guide, career, economy, and Matcha compatibility, so the root artifact also activates seven prototype gameplay systems. MRH-012 must define and obtain approval for the diagnostic-only boundary, and MRH-013 must implement and validate that boundary, before any MRH-011 implementation or packaging work.

  **The existing root-build experiment is behaviour evidence, not a complete fix.** Dropping the existing companion build into a pack instance closes the two measured silent paths, but it does not satisfy the open MRH-010 contract or approve that root artifact:
  ...
  **A cheaper mitigation was tried and does not satisfy the contract.** Global Packs offers `log_pack_ids = true`, which looked like it might expose a wrong archive without shipping any code. It does not: with a healthy Matcha and with a decoy, the `# Listing Data Packs #` block is byte-for-byte identical and mentions Matcha in neither. Configuration alone cannot satisfy MRH-010. MRH-013 must still resolve the locked-hash, distinct-mismatch, role, healthy-noise, and lifecycle evidence gaps before MRH-011 can ship.
- **Custom-code/artifact gate:** No new gameplay code is authorised. MRH-011 may package only the diagnostic-only artifact implemented and validated by MRH-013 after MRH-012 owner approval and completion of MRH-010's acceptance contract; the full root prototype JAR is explicitly not approved because it registers seven prototype systems at common initialization. The existing unit and GameTest evidence is preserved as behaviour evidence, not artifact-scope approval.
- **Explicit non-goals:** No new gameplay, no blocking startup, no unrelated change to the diagnostic's behaviour or wording, no build-architecture decision, and no inclusion or distribution of the full prototype JAR. Wording and evidence changes required to satisfy MRH-010 remain in MRH-013's scoped implementation.
- **Acceptance criteria:**
  1. A pack install using the approved diagnostic-only artifact with Matcha missing shows the framed ERROR banner naming the expected locked SHA-256, and a partially altered real archive shows the WARN/DEGRADED form distinct from Matcha parse errors. The existing decoy result remains recorded as MISSING evidence; one-sided roles are reported where detectable, and a healthy install produces no new diagnostic noise.
...
- **Blocked on:** PACK-01's outstanding human real-play smoke test before inclusion and this ticket's own assembled-pack proof. MRH-012 approval, MRH-013 private dedicated-server validation, and the recorded non-diagnostic baseline evidence are now satisfied; their completion does not authorise wider deployment. PACK-01's final diagnostic criterion remains open and closes only with this ticket's assembled-artifact proof. The human smoke gate is not a prerequisite for MRH-013's internal implementation. The open rights/distribution gates in `docs/decisions/OPEN-rights-model.md` and ADR 0020 also remain. Private-only preparation or a local artifact test does not authorise pack inclusion, publication, or distribution.
...
- **Evidence recorded 2026-08-29 (partial, not a completion claim).** Several acceptance criteria are now demonstrated and should not be re-derived:
  ...
  - **Gap found by that test:** the MRH-010 diagnostic did **not** fire, because the companion mod is not in `pack/`. The safety net exists in `dev.forever.compat.matcha` and has behaviour-level unit and GameTest evidence, but the source-vs-spec acceptance gaps remain open and a player installing the pack today would get the silent degradation MRH-010 was written to prevent. Shipping it needs the MRH-013 diagnostic-only artifact and the rights-model decision, since the pack would then distribute original code. Tracked as **MRH-011**.
  ...
  - Altered-input failure case (criterion 5, tested): a structurally valid decoy archive with the correct filename `Matcha_Flavoured_1_12.zip` produced 1585 recipes and 1688 advancements and **no diagnostic whatsoever**. This is worse than the missing case, because the file is present and correctly named, so nothing looks wrong. It remains historical MISSING evidence for MRH-011's assembled-artifact proof, not completion: MRH-013 must still resolve the source-vs-spec hash, mismatch, role, healthy-noise, and lifecycle gaps.
  - **Still open:** a disposable-world smoke test with real play, which needs a human.

- **Order and criterion status (review correction):** The pinned-input/profile-assembly evidence recorded above is a non-diagnostic prerequisite input to MRH-013; it is not completion of PACK-01. PACK-01's final diagnostic criterion (criterion 5: a clean profile produces an actionable failure for missing or altered inputs) remains **OPEN**. It closes only when MRH-011 proves the approved diagnostic-only artifact in the assembled pack for the missing and partially altered real-archive cases, with the existing decoy/MISSING result retained as historical evidence. Historical LAB-02 and PACK-01 observations are not completion. MRH-013 may perform internal implementation and artifact validation before this human smoke test; the human real-play smoke test remains a gate before MRH-011 pack inclusion. All five PACK-01 acceptance criteria remain in force; none is removed by this ordering note.
